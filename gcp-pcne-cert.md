...
Use the Default Network is a bad practice because of many reasons so that it is better to deactivate its use at the Org Policy Level (Skip Default Network Creation Org Policy)
...
- **Using system-generated default routes**
    ...
    - STo completely isolate a network from the internet or to replace the default route with a custom route, delete the default route:
...
| --- | --- | --- | --- | --- |
| Static Route | - IP range broader than a subnet IP range
- IP range does not overlap with subnet IP range | One of:
- Instance by name
- Instance by IP address
- Cloud VPN tunnel | Yes | Either:
- All instance in network
- Specific instance in network identified by network tag |
| Dynamic Route | - IP range broader than a sbnet IP range
- IP range does not overlap with subnet IP | IP address of the Cloud Router’s BGP peer. | Only by a Cloud Router if it no longer receives router from its BGP peer. | - Instances in the same Region as the Cloud Router if the VPC network is in regional dynamic routing
- If Global-all intances in the VPC |

**************************************************************************Custom versus system-generated routes**************************************************************************

| Type | Destination | Next Hop | Removable | Applies-to |
| --- | --- | --- | --- | --- |
| Custom Static  | - IP range broader than a subnet IP range
- IP range does not overlap with subnet IP range | One of:
- Instance by name
- Instance by IP address
- Cloud VPN tunnel | Yes | Either:
- All instance in network
- Specific instance in network identified by network tag |
| System-generated | - Internet Networks

- Default Gateway - external networks | Internal Subnet 

Gateway | Yes, delete VPC subnet

Yes, Isolate VPC traffic | - All instances in network |
...
- Use BGP which requires a ASN to idenitify different domains
    ...
    - Two modes (configured at the VPC):
        ...
        - Global: Share all the routes in the subnets to and from all regions with a single VPN or interconnect anr Cloud Router
...
- The roles needed to create a VPC peering connection is Project Owner or Editor or NW Admin.
...
- Requires the permissions of Shared VPC Admin (compute.xpnAdmin) at Organization Level
    - During the creation process it will ask to give Service project Admin to specific Roles in the service projects
...
2. Private Service Access
    ...
    - There are only some few services compatible with this approach
        
        ![2023-02-05_576x646.png](images/gcp-pcne/2023-02-05_576x646.png)
...
- Escenarios:
    ...
    - Ephemeral External IP in a VM
        - Change when stop and start the VM
    - Static External IP in a VM
...
- Routes based:
    - A routes-based cluster has a range of IP addresses that are used for Pods and Services. Even though the range is used for both Pods and Services, it is called the Pod address range. The last /20 of the Pod address range is used for Services. A /20 range has 2 power to 12 = 4096 addresses. So 4096 addresses are used for Services, and the rest of the range is used for Pods.
    - Los pods se conectan a través de una puerta de enlace de red, y se les asigna una dirección IP privada única dentro de la subred del clúster. La puerta de enlace de red maneja la comunicación entre los pods y otros servicios en la red. Esta configuración es más fácil de configurar que la VPC-native, pero puede ser menos escalable y tener un menor rendimiento en redes grandes y complejas.
- VPC-native: The ranges are taken from the alias IP Ranges.
    - When selecting the value for the number of pods per node, the value should be between 8 and 110 pods per Node. In order to provide 15 IP Addresses and to support rolling updates of pods, we need at least 30 IP addresses assigned. Using a /27 provides 32 IP addresses for pods per node.
    ...
    - En un clúster VPC-native, los pods se conectan directamente a la red VPC subyacente y se les asignan direcciones IP internas de la red VPC, lo que les permite comunicarse directamente con otros servicios en la red. Esta configuración es recomendada por Google Cloud ya que proporciona un mayor rendimiento y escalabilidad en comparación con los clústeres basados en rutas.
...
The permissions to grant to the GKE Service accounts to use the VPC Networks resources. 
...
- Se habilita cuando creas el cluster (es un flag en Network security)
- Si necesitas añadirlo después tienes que recrear el node pool
- Luego tienes que aplicar un yaml con la configuración de la nw policy.
...
- Esto requiere un esfuerzo de computación con lo cual tienes que redimensionar los clusters.
...
- Takes up to four weeks to be proviisioned
- public advertised prefix (PAP), min range of /24 initially then you can break it
- public delegated prefixes (PDP), a subset of the of PAP configured within a single scope (a specific region or global)

![Untitled](images/gcp-pcne/Untitled%201.png)

- IAM to use the IP address:
...
- Google Cloud has HA and Classic VPN gateways.
    - HA VPN gateways only support dynamic routing with BGP while Classic VPN can be used with [dynamic or static routing](https://cloud.google.com/network-connectivity/docs/vpn/concepts/choosing-networks-routing).
    ...
    - Classic VPN route-based tunnels use 0.0.0.0/0 as local and remote traffic selectors and policy-based tunnels allow configuration of both.
...
- Topologies:
    - **An HA VPN gateway to peer VPN devices.** Require two VPN tunnels from the perspective of the HA VPN gateway.
    ...
    - **Two HA VPN gateways connected to each other. (within Google projects)**
    - **One or more HA VPN gateways associated with two VLAN attachments in an HA VPN over Cloud Interconnect deployment. Each HA VPN gateway is connected to one or more peer VPN devices.**
